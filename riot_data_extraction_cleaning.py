...
with open('contentdriveMyDriveColab Notebooksdatariot_api_key.txt', 'r') as file
    key = file.read().strip()
...
def fetch_data(api_url)
    ...
    if response.status_code == 200
        ...
        print(fError fetching data from API. Status code {response.status_code})
        return None

# Fetch the latest version for static data
version_url = 'httpsddragon.leagueoflegends.comapiversions.json'
versions = fetch_data(version_url)
latest_version = versions[0] if versions else 'latest'

# Fetch champion data
champion_url = f'httpddragon.leagueoflegends.comcdn{latest_version}dataen_USchampion.json'
...
for match_id, game_name, blue_team_name, red_team_name in match_list
    api_url = f'https{server}.api.riotgames.comlolmatchv5matches{match_id}api_key={key}'
    match_data = fetch_data(api_url)

    if match_data
        game_duration_minutes = match_data['info']['gameDuration']  60
        game_duration_display = match_data['info']['gameDuration']  (606024)
        ...
        for participant in match_data['info']['participants']
            ...
            player_info = {
                ...
                'CSM' (participant.get('totalMinionsKilled', 0) + participant.get('neutralMinionsKilled', 0))  game_duration_minutes if game_duration_minutes != 0 else 0,
                'G' participant['goldEarned'],
                'GM' participant['challenges'].get('goldPerMinute', 0),
                'GS' participant['goldEarned']  team_gold_earned if team_gold_earned != 0 else 0,
                'DMG' participant.get('totalDamageDealtToChampions', 0),
                'DMGM' participant.get('totalDamageDealtToChampions', 0)  game_duration_minutes if game_duration_minutes != 0 else 0,
                'DMG%' participant['challenges'].get('teamDamagePercentage', 0),
                'DMGGOLD' participant.get('totalDamageDealtToChampions', 0)  participant.get('goldEarned', 1) if participant.get('goldEarned', 0) != 0 else 0,
                'DT' participant.get('totalDamageTaken', 0),
                'DTD' participant.get('totalDamageTaken', 0)  (participant.get('deaths', 1) if participant.get('deaths', 0) != 0 else 1),
                'DT%' participant['challenges'].get('damageTakenOnTeamPercentage', 0),
                'WARDM' participant.get('wardsPlaced', 0)  game_duration_minutes if game_duration_minutes != 0 else 0,
                'SWEEPM' participant.get('wardsKilled', 0)  game_duration_minutes if game_duration_minutes != 0 else 0,
                'Vision ScoreM' participant['challenges'].get('visionScorePerMinute', 0),
                'FBPAR' FBPAR,
                'WinLose' 'Win' if team_win else 'Lose',
...
credentials = ServiceAccountCredentials.from_json_keyfile_name('contentdriveMyDriveColab Notebooksdatacredentials.json', scope)
